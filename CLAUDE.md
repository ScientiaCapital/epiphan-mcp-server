...
Task tool -> subagent_type: "observer-lite"
  prompt: "Run quality checks on the epiphan-mcp-server codebase. Focus on [relevant area]."

# For STANDARD/FULL scope:
Task tool -> subagent_type: "observer-full"
  prompt: "Run full drift detection on epiphan-mcp-server. The current task is: [describe task]."
```
...
**START DAY:** Start day — project is epiphan-mcp-server. Path: ~/Desktop/tk_projects/epiphan-mcp-server
**FEATURE BUILD:** Feature build — [FEATURE_NAME]
**END DAY:** End day — project is epiphan-mcp-server
